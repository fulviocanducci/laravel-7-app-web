...
class AuthController extends BaseApiController
{
     ...
     *       @OA\Property(property="expires_minute_in", type="integer"),
     *       @OA\Property(
     ...
     *         @OA\Schema(ref="#/components/schemas/auth_access")
     *     )
     *    ),
    ...
    {
        $date_created = new DateTime('now', new DateTimeZone(config('app.timezone')));
        $date_expires = (new DateTime('now', new DateTimeZone(config('app.timezone'))))
            ->add(new DateInterval("PT" . config('jwt.ttl') . "M"));
        $user = auth(AuthController::api)->user();
        return response()->json([
            'access_token' => $token,
            'token_type' => 'bearer',
            'created_at_in' => $date_created,
            'expires_minute_in' => (int)config('jwt.ttl'),
            'expires_date_in' => $date_expires,
